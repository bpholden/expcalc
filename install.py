...
def mk_exec(infiles,output,verb=False):
    for infile in infiles:
        infilename = os.path.basename(infile)
        outfile = os.path.join(output,infilename)
        if os.path.exists(outfile):
            os.chmod(outfile,stat.S_IRWXU | stat.S_IROTH | stat.S_IRWXG | stat.S_IXOTH)
    return
...
try:
    ...
    exit()
filelist = ["web_s2n.wsgi","idl_wrapper"]
try:
    for f in filelist:
        # os.link(f,os.path.join(abspath,f))
        mk_exec(f,abspath,verb)
except Exception as e :
    print "cannot modify permissions for",f,e
    exit()
